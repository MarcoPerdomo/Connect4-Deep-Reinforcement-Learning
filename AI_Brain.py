...
class Network(nn.Module): #Inheriting from Module Parent class
    ...
    def __init__(self, input_size, nb_action): #INitialising the standar function
        ...
        self.fc1 = nn.Linear(input_size, 100) #Creating full connection to the first hidden layer
        self.fc2 = nn.Linear(100, nb_action) # Creating full connection to the output layer
...
class Dqn():
    ...
    def __init__(self, input_size, nb_action, gamma):
        ...
        self.memory = ReplayMemory(10000) # Only one argument, we choose the capacity of the memory
    ...
    def select_action(self, state): #Calling the state (tuple of 5 arguments, 3 sensors, orientation and -orientation) to feed the neural network and obtain the output
        probs = F.softmax(self.model(Variable(state, volatile = True))*1) #Applying softmax to the output of the NN. Do not include the gradient. 
    ...
    def update(self, reward, new_signal): #Makes the connection between the AI and the game. Update the action, last state, last reward as soon as the AI selects an action
        ...
        if len(self.memory.memory) > 1: #first memory is the object of the ReplayMemory class, second memory is the attribute of that class
            batch_state, batch_next_state, batch_action, batch_reward = self.memory.sample(1) #Sampling 100 samples
    ...
    def score(self):
        return sum(self.reward_window)/(len(self.reward_window)+0.00000000000000001) #mean of all the rewards in the reward window (avoiding it equal to 0)
    ...
    def save2(self):#Saving two thigns: self.model and the optimizer
        torch.save({'state_dict': self.model.state_dict(), #saving the parameters as a dictionary. The dictionary needs Key and definitions
                    'optimizer': self.optimizer.state_dict(),
                    }, 'Last_brain2.pth') #Saving in that file
    ...
    def load2(self):
        if os.path.isfile('last_brain2.pth'): #making sure there is a save file
            print("=> loading checkpoint... ")
